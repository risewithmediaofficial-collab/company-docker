...
const Navbar = () => {
  ...
  const handleSearch = (event) => {
    ...
    if (!value) return;
    navigate(`/crm/leads?search=${encodeURIComponent(value)}`);
  ...
  return (
    <header className="sticky top-0 z-40 flex h-16 min-w-0 items-center justify-between border-b border-border bg-card/80 px-4 backdrop-blur-md sm:px-6">
      ...
      <div className="flex min-w-0 flex-1 items-center gap-2 md:gap-4">
        ...
        {/* Search Bar */}
        <div className="relative hidden min-w-0 flex-1 md:block md:max-w-sm lg:max-w-xl">
          <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
            <Search size={18} className="text-muted-foreground" />
          </div>
          <input
            type="text"
            className="block w-full pl-10 pr-3 py-2 border border-border rounded-full bg-secondary/50 text-xs md:text-sm placeholder-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all"
            placeholder="Search..."
